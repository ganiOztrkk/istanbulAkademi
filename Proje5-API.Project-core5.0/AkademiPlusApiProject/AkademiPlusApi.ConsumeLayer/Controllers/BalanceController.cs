...
namespace AkademiPlusApi.ConsumeLayer.Controllers
{
    public class BalanceController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:44367/api/Balance");
        ...
        public async Task<IActionResult> AddBalance(BalanceAddViewModel balanceAddViewModel)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:44367/api/Balance", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> DeleteBalance(int id)
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage =
                await client.DeleteAsync("https://localhost:44367/api/Balance?id=" +
                                         id); // web uygulamalarında ? işareti, işaretten sonra sorgu geleceği anlamına gelir.
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:44367/api/Balance/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateBalance(BalanceUpdateViewModel balanceUpdateViewModel)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:44367/api/Balance/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
